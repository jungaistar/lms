...
// ════════════════════════════════════════════════════════════
//  일자별 기록 — 한 줄에 출결 · 태도 · 과제 · 메모가 다 들어간다
//
//  이 화면은 학생이 들고 오는 기기를 전부 만난다 —
//  아이폰 · 안드로이드 폰 · 아이패드 · 안드로이드 태블릿 · 맥북.
//  교수도 수업 중에는 태블릿으로 본다. 그래서 세 단계로 접힌다.
// ════════════════════════════════════════════════════════════
check(
  '묶음(.mark-group)이 통째로 접힌다',
  /\.mark-group \{[^}]*flex-direction: column;/.test(flat),
  '라벨과 버튼이 한 묶음이어야 한다. 가로로 두면 좁은 화면에서 라벨만 남고 버튼이 다음 줄로 떨어진다.',
);
check(
  '태블릿에서 묶음 두 개가 한 줄에 선다',
  /\.mark-group, \.mark-group-wide \{ flex: 1 1 45%; \}/.test(m900),
  '아이패드 세로 · 안드로이드 태블릿이 여기 걸린다. 한 줄에 하나만 두면 스크롤이 길어진다.',
);
check(
  '폰에서 묶음이 한 줄을 통째로 쓴다',
  /\.mark-group, \.mark-group-wide \{ flex: 1 1 100%; \}/.test(m640),
  '360px 에서 출결 다섯 개와 점수 버튼이 같은 줄에 들어갈 수 없다.',
);
check(
  '폰에서 출결 버튼이 폭을 고르게 나눈다 (일자별 기록)',
  /\.mark-group > \.btn-row > \.btn-sm \{ flex: 1 1 0;/.test(m640),
  '수업 중에 손가락으로 누르는 자리다.',
);
check(
  '폰에서 점수 버튼이 44px 아래로 안 내려간다',
  /\.mark-group > \.score-picker > button \{[^}]*min-height: 44px;/.test(m640),
  '터치 대상은 44px 이상이어야 한다. 0~5 를 여섯 개 늘어놓으면 쉽게 이보다 작아진다.',
);
