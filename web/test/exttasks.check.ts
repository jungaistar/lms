...
// ── 이름 고르기 ──────────────────────────────────────────────
//
// 2026-08-12 에 실제로 터진 버그: 학번 앞의 "학부" 칸이 이름으로 잡혀
// 모든 학생 이름이 "학부" 가 됐다. 학번 **다음** 칸을 먼저 봐야 한다.
{
  const cells = ['1', '학부', '3', '202126002', '고재욱', '', '3/3', '조회'];
  check('이름: 학번 앞의 "학부" 를 이름으로 잡지 않는다', pickName(cells, '202126002') === '고재욱', pickName(cells, '202126002'));
}
check('이름: "이름 학번" 순서면 앞쪽에서 찾는다', pickName(['고재욱', '202126002'], '202126002') === '고재욱');
check('이름: 학번 뒤가 상태 낱말이면 넘어간다', pickName(['202126002', '미제출'], '202126002') === null, pickName(['202126002', '미제출'], '202126002'));
check('이름: 학년·학과 같은 낱말도 거른다', pickName(['학과', '202126002', '전공', '김서현'], '202126002') === '김서현');

{
  const r = parseTaskCounts('1\t학부\t3\t202126002\t고재욱\t\t3/3\t조회');
  check('제출수: 학교명 칸이 있어도 이름을 제대로 읽는다', r.rows[0]?.name === '고재욱', r.rows[0]?.name);
}
