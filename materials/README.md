...
```
materials/
  theme.css            디자인 시스템 (학교 자체 강의자료의 시각 언어)
  assets/              배경 사진 (학교 자료에서 가져옴 — assets/README.md)
...
『창업과 기업가 정신』 1강 오리엔테이션(학교 자체 강의자료)의 시각 언어를 따른다.
색은 그 파일의 `srgbClr` 값을 그대로 옮겼고, 배경 사진도 그 파일의 것을 쓴다.

| | |
|---|---|
| 헤더 | 56px 파랑 그라디언트 띠 + **노란 테를 두른 섹션 칩** + dima 워드마크 |
| 본문 | 옅은 캠퍼스 사진 위, 제목은 남색 굵게 (**밑줄 없음**) |
| 강조 | 흰 상자 + **왼쪽 굵은 색막대** (파랑=안내, 주황=참고, 빨강=경고) |
| 카드 | 옅은 파랑 면 + 파란 1px 테 (둥근 모서리 없음) |
| 챕터 표지 | 사진 띠 + 왼쪽 원형 배지 + **제목은 띠 아래 오른쪽** |
| 표 | 옅은 파랑 머리 + 파란 굵은 숫자, 과제물은 빨강 |
...
- **한 요소 안에 블록 자식과 맨 글자를 섞지 말 것.** 좌표를 뽑을 때 맨 글자가
  통째로 빠진다 (헤더 칩의 섹션 이름이 사라졌던 자리다). 글은 반드시 제 태그로
  감쌀 것 — `.slide__chiptext` · `.h-sub__text` 가 그래서 있다.
- `.callout` 은 flex 다. 글을 `.callout__body` 로 감싸지 않으면 `<b>`·`<em>`
  마다 따로 떨어져 나가 문장이 조각난다.

## 고칠 때

- **글을 고친다** → `week01/slides.mjs`. 다섯 가지 산출물이 같이 바뀐다.
- **배경을 고친다** → `assets/`. `build.mjs` 의 `ASSETS` 가 목적지마다
  다르게 심는다 (HTML=data URI, 캔버스=파일, PPTX=그림).
